...
public class ActorEditProfilesPage extends AbstractPageContainerManagedImpl implements Serializable {
	...
	protected String __getWindowTitleValue__() {
		return Helper.formatActorListPrivilegesWindowTitle(actor,null);
	}
	...
	protected void __listenPostConstruct__() {
		...
		if(Boolean.TRUE.equals(isRenderTypeDialog)) {
			...
			cellsMaps.add(MapHelper.instantiate(Cell.FIELD_CONTROL,Helper.buildActorAutoCompleteRedirector(actor, "actorEditProfilesView"),Cell.FIELD_WIDTH,10));
		}
		...
		if(actor == null) {
			...
			tree = PrivilegeTreePage.buildTree(Tree.FIELD_VALUE,PrivilegeTreePage.instantiateTreeNode(privileges,null));	
			
			cellsMaps.addAll(CollectionHelper.listOf(
								...
								protected Object __runExecuteFunction__(AbstractAction action) {
									...
									if(CollectionHelper.isNotEmpty(arguments.getCreatables()) || CollectionHelper.isNotEmpty(arguments.getDeletables())) {
										...
										if(!Boolean.TRUE.equals(isRenderTypeDialog)) {
											...
											Redirector.getInstance().redirect("actorEditProfilesView", map);
										}

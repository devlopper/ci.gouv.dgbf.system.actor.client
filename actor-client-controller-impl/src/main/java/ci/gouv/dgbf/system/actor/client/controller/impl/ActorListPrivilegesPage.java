	...
	protected void addDataComponent(Collection<Map<?, ?>> cellsMaps) {
		if(actor != null) {
			...
			if(CollectionHelper.isNotEmpty(privilegeTypes)) {				
				...
				availableTree = Tree.build(Tree.FIELD_VALUE,PrivilegeListPage.instantiateTreeNode(availablePrivileges,selectedPrivileges),Tree.ConfiguratorImpl.FIELD_PROCESS_SELECTION_COMMAND_BUTTON_INSTANTIABLE,Boolean.TRUE
					...
					public void processSelection(Tree tree,Collection<Privilege> privileges) {
						...
						if(ActorListPrivilegesPage.this.profilePrivileges == null)
							ActorListPrivilegesPage.this.profilePrivileges = new ArrayList<>();
						ActorListPrivilegesPage.this.profilePrivileges.addAll(profilePrivileges);
						LogHelper.logInfo(CollectionHelper.getSize(ActorListPrivilegesPage.this.profilePrivileges)+" profile privilege(s) to show", getClass());
